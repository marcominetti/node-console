...
var Exception = global.Exception || function(error){
...
(function() {
    this.evaluate = function (params, sendResult) {
      ...
      try {
          ...
          var comments = [];
          var tokens = [];
          var code = esnext_prefix + params.expression + esnext_suffix;
          var ast = require('acorn-babel-codetag').parse(code, {
          ...
          require('estraverse').attachComments(ast,comments,tokens);
          ...
          if (last_statement_container.body.length === 0 || (last_statement_node.type !== 'ExpressionStatement' && last_statement_node.type !== 'ReturnStatement')) {
          ...
          } else if (last_statement_node.type === 'ExpressionStatement' && last_statement_node.type !== 'ReturnStatement'){
            last_statement_container.body[last_statement_index] = {
                "type": "ReturnStatement",
                "argument": last_statement_node.expression
            };
          }
          var generated = require('babel').transform.fromAst(ast,null,{
            'experimental': true,
            'playground': false,
            'format': {
              'parentheses': true,
          ...
          //result = eval.call(global, "with ({}) {\n" + params.expression + "\n}");
            ...
            sendResult(self.createThrownValue(e, params.objectGroup));
        }
